...
tech_edu<-ggpredict(lm4, terms=c("Positive_tech [1,3,5,7,10]", "Social_media_usage [1,5]"))
footnote_tech_edu <- str_wrap("Note: Views on technological impact in regards to long term benefits to humanity in health, comfort, ease of life, and opportunites for future generations, social media usage set to daily and never", width = 70)
...
ggplot(tech_edu, aes(x = factor(x, levels = 1:10), 
  ...
  scale_fill_manual(name = "Social Media Usage",
                    values = c("lightgrey", "black"),
                    labels = c("1" = "Daily", "5" = "Never")) +  # Update legend labels
...
tech_edu<-ggpredict(lm4, terms=c("Inter_trust [1:4, by=1]", "Social_media_usage [1,5]"))
footnote_inter_edu <- str_wrap("Note: Trust in strangers, people of another religion, and nationality, social media usage set to daily and never", width = 70)
...
edu_tab <- moral %>%  
  ...
  mutate(Response = case_when(
     is.na(Q275) ~ "No Response", 
    Q275 == 0 ~ "Early childhood edu",     
    Q275 == 1 ~ "Primary",    
    Q275 == 2 ~ "LoIr secondary",           
    Q275 == 3 ~ "Upper secondary",
    Q275 == 4 ~ "Post-secondary",
    Q275 == 5 ~ "Short-cycle tertiary",
    Q275 == 6 ~ "Bachelors",
    Q275 == 7 ~ "Masters",
    Q275 == 8 ~ "Doctoral",
    TRUE ~ as.character(Q275)          
  )) %>% 
  rename(Count = n, Percent = percent) %>% 
 dplyr::select(-matches("valid_percent")) 

e<-flextable(edu_tab) #Save the tibble as a flextable
e

age_avg<- moral %>% 
  summarise(mean = mean(Q262, na.rm = TRUE),  # Exclude NA values
            sd = sd(Q262, na.rm = TRUE),      # Exclude NA values
            n = sum(!is.na(Q262))) %>%        # Count non-NA values
...
#my sample consisted of (n=2,412) Americans with a mean age of 43.5 and majority educated (post-secondary = 25% and bachelors = 25%, masters = 11.8%, and doctoral = 5%)
...
social_tab <- moral %>%  
  ...
  filter(n != 0) %>% 
    mutate(percent = round(percent * 100, 1), 
         percent = paste0(percent, "%"))  %>% 
  mutate(Response = case_when(
     is.na(Q207) ~ "No Response",
    Q207 == 1 ~ "Daily",    
    Q207 == 2 ~ "Weekly",           
    Q207 == 3 ~ "Monthly",
    Q207 == 4 ~ "Less than monthly",
    Q207 == 5 ~ "Never",
    TRUE ~ as.character(Q207)          
  )) %>% 
  rename(Count = n, Percent = percent) %>% 
 dplyr::select(-matches("valid_percent"))
 
s<-flextable(social_tab) #Save the tibble as a flextable
s
```
